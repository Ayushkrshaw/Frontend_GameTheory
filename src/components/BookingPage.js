...
function BookingPage() {
  ...
  const fetchCenters = async () => {
    try {
      const response = await axios.get(`https://backend-gametheory.onrender.com/api/centers`);
  ...
  const fetchSports = async (centerId) => {
    try {
      const response = await axios.get(`https://backend-gametheory.onrender.com/api/sports?center=${centerId}`);
  ...
  const fetchBookings = async () => {
    try {
      const response = await axios.get(`https://backend-gametheory.onrender.com/api/bookings?center=${selectedCenter}&sport=${selectedSport}&date=${selectedDate}`);
  ...
  const fetchAvailableSlots = async (court) => {
    ...
    if (selectedCenter && selectedSport && selectedDate) {
      try {
        const response = await axios.get(`https://backend-gametheory.onrender.com/api/bookings/available-slots`, {
          params: {
  ...
  const handleCreateBooking = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`https://backend-gametheory.onrender.com/api/bookings`, {
        center: selectedCenter,
        sport: selectedSport,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      {/* Main Content */}
      <main className="flex-grow space-y-12 px-8 py-10 max-w-7xl mx-auto">
        {/* Booking Details Section */}
        <div className="bg-white p-8 rounded-xl shadow-xl transition duration-300 transform hover:-translate-y-1 hover:shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <select
              ...
              className="border-2 border-indigo-300 p-3 rounded-lg text-gray-700 focus:outline-none focus:ring-4 focus:ring-indigo-300 transition duration-200"
            >
          ...
          <button
            ...
            className="mt-8 w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold text-lg px-6 py-3 rounded-lg shadow-lg hover:from-indigo-600 hover:to-purple-700 transform hover:-translate-y-1 transition duration-300 ease-in-out"
          >
            View Bookings
          </button>
        </div>
...
<h2 className="text-3xl font-extrabold text-black mb-8 text-center drop-shadow-md">
  ...
  <div className="flex overflow-x-auto pb-4 space-x-6">
    {bookings.map((booking) => (
      <div
        key={booking._id}
        className="bg-gray-50 border border-gray-200 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 transform hover:scale-105 w-64"
      >
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl transition duration-300 transform hover:-translate-y-1 hover:shadow-2xl">
          ...
          <form onSubmit={handleCreateBooking} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-green-400 to-teal-500 text-white font-semibold text-lg px-6 py-3 rounded-lg shadow-lg hover:from-green-500 hover:to-teal-600 transform hover:-translate-y-1 transition duration-300 ease-in-out"
            >
              Create Booking
            </button>
          </form>
